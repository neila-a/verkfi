...
const installStaticFiles = () => caches.open(Cache).then(cache => cache.addAll(installFilesEssential).catch(reason => console.error(reason))); // install static assets
...
self.addEventListener('fetch', event => {
    if (event.request.method !== 'GET') return;reason
